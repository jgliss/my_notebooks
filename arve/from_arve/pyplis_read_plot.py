...
if __name__=="__main__":
    ...
    if PlotType=='Velocity':
        ...
        AddWhiteNise=True #False#
        if AddWhiteNise:
            for im in ImagesAA:
                im.img = im.img  + 0.4 * im.img.std() * np.random.random(im.img.shape)
        ...
        figs.extend([ax0.figure, fig0, fig])
        ...
        for k in range(len(figs)):
            figs[k].savefig(join(SAVE_DIR, "%s_out_%s_%d.%s"
                                 %(SCRIPT_ID, case_here, (k+1), SAVE_PLOT_TYPE)),
                            dpi=SAVE_PLOT_DPI)
